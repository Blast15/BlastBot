...
async def handle_command_error(interaction: discord.Interaction, error: Exception):
    ...
    # Expected user/permission errors logged at info level without stack trace
    expected_errors = (
    ...
    if isinstance(error, expected_errors):
        logger.info(
            f"Command '{command_name}' by {interaction.user} triggered expected error: {error}"
        )
    else:
        logger.error(
...
def validate_member_hierarchy(
    moderator: discord.Member, target: discord.Member, bot_member: discord.Member
) -> tuple[bool, str | None]:
    """
    ...
    if target.top_role >= moderator.top_role:
        return (
            False,
            "❌ Bạn không thể thực hiện hành động này với member có role cao hơn hoặc bằng bạn!",
        )
    ...
    if target.top_role >= bot_member.top_role:
        return (
            False,
            "❌ Bot không thể thực hiện hành động này với member có role cao hơn hoặc bằng bot!",
        )
...
def validate_number_range(
    value: int, min_val: int, max_val: int, param_name: str = "Giá trị"
) -> int:
...
def normalize_channel_name(name: str) -> str:
    """Normalize channel name to valid discord channel slug"""
    value = name.strip().lower().replace(" ", "-")
    value = re.sub(r"[^a-z0-9-_]", "-", value)
    value = re.sub(r"-{2,}", "-", value).strip("-")
    return value[:100] or "ticket"
