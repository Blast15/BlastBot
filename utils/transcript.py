...
async def generate_transcript(channel: discord.TextChannel, limit: int = 2000) -> discord.File:
    ...
    limit_warning = f"<p style='color:#faa61a;'>⚠️ Chỉ hiển thị {limit} tin nhắn gần nhất.</p>" if len(messages) >= limit else ""
...
<p>Số tin nhắn: {len(messages)}</p>{limit_warning}<hr>{body}</body></html>"""
    return discord.File(io.BytesIO(doc.encode('utf-8')),
