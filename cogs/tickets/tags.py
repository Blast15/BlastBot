...
class TicketTags(commands.Cog):
    ...
    managetags = app_commands.Group(
        name="managetags",
        description="Quản lý ticket tags",
    ...
    async def tag_cmd(self, interaction: discord.Interaction, tag_id: str):
        ...
        if not isinstance(
            ...
            return await interaction.response.send_message(
                "❌ Chỉ staff mới dùng được tag.", ephemeral=True
            )

        content = await self.bot.db.get_tag(interaction.guild.id, tag_id)
        if not content:
            return await interaction.response.send_message(
                embed=error_embed("Lỗi", f"Không tìm thấy tag `{tag_id}`."),
                ephemeral=True,
            )
    ...
    async def tag_add(
        ...
        await interaction.response.send_message(
            embed=success_embed("Đã lưu tag", f"Tag `{clean_id}` đã sẵn sàng sử dụng."),
            ephemeral=True,
        )
    ...
    async def tag_delete(self, interaction: discord.Interaction, tag_id: str):
        ...
        if deleted:
            await interaction.response.send_message(
                embed=success_embed("Đã xóa tag", f"Đã xóa tag `{clean_id}`."),
                ephemeral=True,
            )
        else:
            await interaction.response.send_message(
                embed=error_embed("Lỗi", f"Không tìm thấy tag `{clean_id}`."),
                ephemeral=True,
            )
    ...
    async def tag_list(self, interaction: discord.Interaction):
        ...
        if not tags:
            return await interaction.response.send_message(
                embed=create_embed(
                    title="Tags", description="Chưa có tag nào.", color=COLORS["info"]
                ),
                ephemeral=True,
            )
