...
from utils.error_handler import validate_number_range, validate_string_length, ValidationError
...
class AutoMessage(commands.Cog):
    ...
    automsg = app_commands.Group(
        name="automsg", description="Quản lý tin nhắn tự động lặp lại",
        default_permissions=discord.Permissions(manage_guild=True),
        guild_only=True)

    @automsg.command(name="add", description="Thêm auto-message")
    @app_commands.describe(channel="Channel gửi tin", content="Nội dung tin nhắn",
                           interval="Chu kỳ (phút)", use_embed="Gửi dạng embed?")
    ...
    async def add(self, interaction: discord.Interaction, channel: discord.TextChannel,
                  content: str, interval: int, use_embed: bool = False):
        if interaction.guild is None:
            return
        try:
            content = validate_string_length(content, min_len=1, max_len=4000 if use_embed else 2000)
            validate_number_range(
                interval, AUTOMATION_CONFIG['min_interval_minutes'],
                AUTOMATION_CONFIG['max_interval_minutes'], "Chu kỳ (phút)")
    ...
    async def list_cmd(self, interaction: discord.Interaction):
        ...
        if not items:
            return await interaction.response.send_message(
                embed=create_embed(title="Auto-messages", description="Chưa có cái nào.",
                                   color=COLORS['info']), ephemeral=True)
        lines = []
        for m in items:
            status = "🟢" if m['enabled'] else "🔴"
            preview = (m['content'][:40] + "…") if len(m['content']) > 40 else m['content']
            lines.append(f"{status} `{m['id']}` — <#{m['channel_id']}> mỗi "
                         f"{m['interval_minutes']}p: {preview}")
        await interaction.response.send_message(
            embed=create_embed(title="📨 Auto-messages", description="\n".join(lines),
                               color=COLORS['info']), ephemeral=True)
    ...
    async def delete(self, interaction: discord.Interaction, auto_id: int):
        ...
        embed = (success_embed("Đã xóa", f"Đã xóa auto-message `{auto_id}`.") if ok
    ...
    @require_guild_permissions(manage_guild=True)
    async def toggle(self, interaction: discord.Interaction, auto_id: int, enabled: bool):
        if interaction.guild is None:
            return
        ok = await self.bot.db.toggle_auto_message(interaction.guild.id, auto_id, enabled)
        embed = (success_embed("Đã cập nhật",
                    f"Auto-message `{auto_id}` đã {'bật' if enabled else 'tắt'}.") if ok
                 else error_embed("Lỗi", f"Không tìm thấy auto-message `{auto_id}`."))
        await interaction.response.send_message(embed=embed, ephemeral=True)
