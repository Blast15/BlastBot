...
from utils.constants import COLORS, AUTOMATION_CONFIG
from utils.placeholders import render_placeholders
from utils.error_handler import validate_string_length, ValidationError
...
class Greetings(commands.Cog):
    ...
    greeting = app_commands.Group(
        name="greeting", description="Cấu hình lời chào/tạm biệt",
    ...
    async def _send_greeting(self, member: discord.Member, kind: str):
        ...
        message = render_placeholders(
            cfg['message'] or ("Chào mừng {user_mention} đến với **{server}**! 👋"
                               if kind == 'welcome'
    ...
    async def _setup_cmd(self, interaction, kind, channel, message, title, use_embed):
        if interaction.guild is None:
            return
        try:
            message = validate_string_length(message, min_len=1, max_len=4000 if use_embed else 2000)
            if title:
        ...
        await self.bot.db.set_greeting(
            interaction.guild.id, kind, enabled=1, channel_id=channel.id,
            message=message, title=title, use_embed=int(use_embed),
    ...
    @app_commands.describe(channel="Channel gửi lời chào", message="Nội dung (hỗ trợ placeholder)",
                           title="Tiêu đề embed (tùy chọn)", use_embed="Gửi dạng embed?")
    ...
    async def welcome(self, interaction: discord.Interaction, channel: discord.TextChannel,
                      message: str, title: str | None = None, use_embed: bool = True):
        await self._setup_cmd(interaction, 'welcome', channel, message, title, use_embed)

    @greeting.command(name="goodbye", description="Bật & cấu hình lời tạm biệt")
    @app_commands.describe(channel="Channel gửi lời tạm biệt", message="Nội dung (hỗ trợ placeholder)",
                           title="Tiêu đề embed (tùy chọn)", use_embed="Gửi dạng embed?")
    @require_guild_permissions(manage_guild=True)
    async def goodbye(self, interaction: discord.Interaction, channel: discord.TextChannel,
                      message: str, title: str | None = None, use_embed: bool = True):
        await self._setup_cmd(interaction, 'goodbye', channel, message, title, use_embed)
    ...
    @require_guild_permissions(manage_guild=True)
    async def disable(self, interaction: discord.Interaction, kind: app_commands.Choice[str]):
        if interaction.guild is None:
            return
        await self.bot.db.set_greeting(interaction.guild.id, kind.value, enabled=0)
        await interaction.response.send_message(
            embed=success_embed("Đã tắt", f"Đã tắt {kind.name.lower()}."), ephemeral=True)

    @greeting.command(name="test", description="Gửi thử lời chào/tạm biệt cho chính bạn")
    @app_commands.choices(kind=[
        app_commands.Choice(name="Lời chào", value="welcome"),
        app_commands.Choice(name="Lời tạm biệt", value="goodbye"),
    ])
    @require_guild_permissions(manage_guild=True)
    async def test(self, interaction: discord.Interaction, kind: app_commands.Choice[str]):
        if interaction.guild is None or not isinstance(interaction.user, discord.Member):
            return
        await self._send_greeting(interaction.user, kind.value)
        await interaction.response.send_message(
            embed=success_embed("Đã gửi thử", "Kiểm tra channel đã cấu hình nhé."), ephemeral=True)
