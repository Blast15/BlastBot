...
from blastbot.shared.embeds import success
...
class SuggestionVotingView(SafeView):
    ...
    async def _vote(self, interaction: discord.Interaction, value: int) -> None:
        if interaction.message is None:
            await interaction.response.send_message("Không xác định được suggestion.", ephemeral=True)
            return
        up, down = await self.service.vote(interaction.message.id, interaction.user.id, value)
...
class SuggestionModal(SafeModal, title="Gửi góp ý"):
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        ...
        card = discord.Embed(title="💡 Góp ý mới", description=text, color=discord.Color.blurple())
        card.set_author(name=str(interaction.user), icon_url=interaction.user.display_avatar.url)
...
class ReportModal(SafeModal, title="Báo cáo người dùng/tin nhắn"):
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        reason, details = self.service.validate_report(str(self.reason.value), str(self.details.value))
        lines = [
        ...
        await interaction.response.send_message(
            embed=success(
                "Báo cáo đã gửi", "Cảm ơn bạn đã báo cáo. Đội ngũ quản lý sẽ xem xét sớm nhất."
            ),
            ephemeral=True,
        )
