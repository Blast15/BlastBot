...
class TicketCog(commands.Cog):
    ticket = app_commands.Group(
        name="ticket",
        description="Thao tác trong ticket",
        guild_only=True,
    )
    ticketconfig = app_commands.Group(
        name="ticket-config",
    ...
    async def _require_ticket_staff(
        ...
        await interaction.response.send_message(
            embed=error("Không đủ quyền", "Bạn không phải ticket staff."),
            ephemeral=True,
        )
        return None
    ...
    async def _current_ticket(self, interaction: discord.Interaction) -> Ticket | None:
        if not isinstance(interaction.channel, discord.TextChannel):
            return None
        return await self.bot.app.tickets_repo.get_ticket_by_channel(
            interaction.channel.id
        )

    @ticketconfig.command(name="transcripts", description="Đặt channel lưu transcript")
    ...
    async def transcripts(
        ...
        if interaction.guild_id is None:
            return
        await self.bot.app.tickets.set_transcript_channel(
            interaction.guild_id, channel.id
        )
        await interaction.response.send_message(
            embed=success(
                "Đã cập nhật", f"Transcript sẽ được gửi vào {channel.mention}."
            ),
            ephemeral=True,
        )

    @ticketconfig.command(
        name="limit", description="Đặt số ticket tối đa mỗi thành viên"
    )
    @require_guild_permissions(manage_guild=True)
    async def limit(
    ...
    @ticketconfig.command(
        name="autoclose",
        description="Tự đóng ticket không hoạt động sau N giờ (0 = tắt)",
    )
    @require_guild_permissions(manage_guild=True)
    async def autoclose(
        ...
        if interaction.guild_id is None:
            return
        value = await self.bot.app.tickets.set_autoclose(
            interaction.guild_id, int(hours)
        )
        message = (
            "Autoclose đã tắt."
            if value == 0
            else f"Ticket sẽ tự đóng sau **{value} giờ** không hoạt động."
        )
        await interaction.response.send_message(
            embed=success("Đã cập nhật", message), ephemeral=True
        )

    @ticketconfig.command(
        name="claimmode", description="Chọn hành vi sau khi staff claim ticket"
    )
    @app_commands.choices(
        mode=[
            app_commands.Choice(
                name="Chỉ người claim được trả lời", value="reply_only"
            ),
    ...
    @ticketconfig.command(
        name="addsupport", description="Thêm user hoặc role vào ticket staff"
    )
    @require_guild_permissions(manage_guild=True)
    async def addsupport(
        ...
        if interaction.guild_id is None or (member is None) == (role is None):
            await interaction.response.send_message(
                embed=error(
                    "Input không hợp lệ", "Chọn đúng một member hoặc một role."
                ),
                ephemeral=True,
            )
            return
        ...
        if isinstance(target, discord.Role) and target.is_default():
            await interaction.response.send_message(
                embed=error(
                    "Role không hợp lệ",
                    "Không thể dùng role @everyone làm ticket staff.",
                ),
                ephemeral=True,
            )
            return
        ...
        await interaction.response.send_message(
            embed=success("Đã thêm support", target.mention),
            ephemeral=True,
            allowed_mentions=discord.AllowedMentions.none(),
        )

    @ticketconfig.command(
        name="removesupport", description="Xóa user hoặc role khỏi ticket staff"
    )
    @require_guild_permissions(manage_guild=True)
    async def removesupport(
        ...
        if target is None:
            return
        await self.bot.app.tickets_repo.remove_staff(
            interaction.guild_id, target.id, "support"
        )
        await interaction.response.send_message(
            embed=success("Đã xóa support", target.mention),
            ephemeral=True,
            allowed_mentions=discord.AllowedMentions.none(),
        )

    @ticketconfig.command(name="viewstaff", description="Xem danh sách ticket staff")
    @require_guild_permissions(manage_guild=True)
    async def viewstaff(self, interaction: discord.Interaction) -> None:
        ...
        await interaction.response.send_message(
            embed=info("Ticket staff", text), ephemeral=True
        )

    @ticketconfig.command(
        name="blacklist", description="Bật/tắt blacklist ticket cho member hoặc role"
    )
    @require_guild_permissions(manage_guild=True)
    async def blacklist(
        self,
    ...
    async def panel_send(
        ...
        embed = discord.Embed(
            title=panel.title, description=panel.content, color=panel.color
        )
        ...
        await interaction.response.send_message(
            embed=success(
                "Đã gửi panel", f"Panel `{panel_id}` đã gửi tại {message.jump_url}."
            ),
            ephemeral=True,
        )
    ...
    async def panel_edit(
        ...
        await interaction.response.send_message(
            embed=success(
                "Đã cập nhật",
                "Panel đã thay đổi." if changed else "Không có trường nào thay đổi.",
            ),
            ephemeral=True,
        )
    ...
    async def panel_delete(
        ...
        await interaction.response.send_message(
            embed=success("Đã xóa panel", f"Panel `{panel_id}` đã bị xóa."),
            ephemeral=True,
        )

    @ticket.command(name="add", description="Thêm thành viên vào ticket hiện tại")
    async def add_member(
        self, interaction: discord.Interaction, member: discord.Member
    ) -> None:
        if await self._require_ticket_staff(interaction) is None or not isinstance(
        ...
        await interaction.response.send_message(
            embed=success("Đã thêm", f"Đã thêm {member.mention} vào ticket."),
            allowed_mentions=discord.AllowedMentions.none(),
        )

    @ticket.command(name="remove", description="Xóa thành viên khỏi ticket hiện tại")
    async def remove_member(
        ...
        await interaction.response.send_message(
            embed=success("Đã xóa", f"Đã xóa {member.mention} khỏi ticket."),
            allowed_mentions=discord.AllowedMentions.none(),
        )

    @ticket.command(name="rename", description="Đổi tên ticket hiện tại")
    async def rename(self, interaction: discord.Interaction, name: str) -> None:
        ...
        if await self._current_ticket(interaction) is None:
            await interaction.response.send_message(
                embed=error("Không phải ticket", "Channel này không phải ticket."),
                ephemeral=True,
            )
            return
        safe = "".join(
            ch for ch in name.lower().replace(" ", "-") if ch.isalnum() or ch == "-"
        )[:90]
        if not safe:
            await interaction.response.send_message(
                embed=error("Tên không hợp lệ", "Tên ticket không được rỗng."),
                ephemeral=True,
            )
            return
        await interaction.channel.edit(
            name=safe, reason=f"Ticket renamed by {interaction.user}"
        )
        await interaction.response.send_message(
            embed=success("Đã đổi tên", f"Tên mới: `{safe}`."), ephemeral=True
        )

    @ticket.command(name="transfer", description="Chuyển ownership của ticket")
    async def transfer(
        self, interaction: discord.Interaction, member: discord.Member
    ) -> None:
        if await self._require_ticket_staff(interaction) is None or not isinstance(
        ...
        if ticket is None or not ticket.open:
            ...
            return
        old_owner = (
            interaction.guild.get_member(ticket.owner_id) if interaction.guild else None
        )
        if old_owner:
        ...
        await self.bot.app.tickets_repo.transfer_owner(
            interaction.channel.id, member.id
        )
        await interaction.response.send_message(
            embed=success("Đã transfer", f"Owner mới: {member.mention}."),
            allowed_mentions=discord.AllowedMentions.none(),
        )

    @ticket.command(name="tag", description="Gửi quick tag trong ticket")
    async def tag(self, interaction: discord.Interaction, tag_id: str) -> None:
        if (
            interaction.guild_id is None
            or await self._require_ticket_staff(interaction) is None
        ):
            return
        content = await self.bot.app.tickets.tag(interaction.guild_id, tag_id)
        if content is None:
            await interaction.response.send_message(
                embed=error("Không tìm thấy tag", f"Không có tag `{tag_id}`."),
                ephemeral=True,
            )
            return
        await interaction.response.send_message(
    ...
    @require_guild_permissions(manage_guild=True)
    async def tags_add(
        self, interaction: discord.Interaction, tag_id: str, content: str
    ) -> None:
        if interaction.guild_id is None:
            return
        normalized = await self.bot.app.tickets.add_tag(
            interaction.guild_id, tag_id, content
        )
        await interaction.response.send_message(
            embed=success("Đã lưu tag", f"Tag: `{normalized}`."), ephemeral=True
        )
    ...
    async def tags_delete(self, interaction: discord.Interaction, tag_id: str) -> None:
        ...
        await interaction.response.send_message(
            embed=success("Quick tags", "Đã xóa." if deleted else "Tag không tồn tại."),
            ephemeral=True,
        )
    ...
    async def tags_list(self, interaction: discord.Interaction) -> None:
        ...
        await interaction.response.send_message(
            embed=info(
                "Quick tags", "\n".join(f"`{tag}`" for tag in tags) or "Chưa có tag."
            ),
            ephemeral=True,
        )

    @ticket.command(
        name="close-request", description="Yêu cầu staff xác nhận đóng ticket"
    )
    async def close_request(self, interaction: discord.Interaction) -> None:
        ...
        if interaction.user.id != ticket.owner_id:
            await interaction.response.send_message(
                embed=error("Không đủ quyền", "Chỉ chủ ticket mới dùng lệnh này."),
                ephemeral=True,
            )
            return
        await interaction.response.send_message(
            embed=info(
                "Yêu cầu đóng ticket",
                f"{interaction.user.mention} yêu cầu đóng ticket. Staff hãy xác nhận.",
            ),
    ...
    @ticket.command(
        name="exclude-autoclose", description="Loại ticket hiện tại khỏi autoclose"
    )
    async def exclude_autoclose(self, interaction: discord.Interaction) -> None:
        ...
        await interaction.response.send_message(
            embed=success(
                "Đã loại khỏi autoclose", "Ticket này sẽ không bị đóng do inactivity."
            ),
            ephemeral=True,
        )
    ...
    async def autoclose_loop(self) -> None:
        ...
        for ticket in rows:
            ...
            if not isinstance(channel, discord.TextChannel):
                await self.bot.app.tickets_repo.close_ticket(
                    ticket.channel_id, "Channel không còn tồn tại"
                )
                continue
            ...
            try:
                await perform_close(
                    ...
                    channel,
                    actor=bot_member,
                    reason="Autoclose do inactivity",
                    system=True,
                )
